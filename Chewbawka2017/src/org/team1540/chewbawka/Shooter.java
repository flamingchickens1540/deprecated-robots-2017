...
////		flywheelMotorRight.modGeneralConfig().activateFollowerMode(flywheelMotorLeft);
		...
		flywheelMotorLeft.modPID().getP().set(Robot.mainTuning.getFloat("Shooter Flywheel PID P", 0.01f).get());
		flywheelMotorLeft.modPID().getI().set(Robot.mainTuning.getFloat("Shooter Flywheel PID I", 0.01f).get());
		flywheelMotorLeft.modPID().getD().set(Robot.mainTuning.getFloat("Shooter Flywheel PID D", 0.01f).get());
		FloatOutput flywheelSpeedControl = flywheelMotorLeft.asMode(OutputControlMode.SPEED_FIXED);
		FloatInput flywheelSpeed = flywheelMotorLeft.modEncoder().getEncoderVelocity().absolute();
		FloatInput flywheelShootingSpeed = Robot.mainTuning.getFloat("Shooter Flywheel Shooting Speed", 3600f);
//		BooleanInput flywheelUpToSpeed = flywheelVelocity.atLeast(flywheelTargetSpeed);
//		ControlBindings.spinupButton.toFloat(0f, flywheelTargetSpeed.negated()).send(flywheelSpeedControl);
		
//		Cluck.publish("Shooter Flywheel Position", flywheelMotorLeft.modEncoder().getEncoderPosition());
...
//		
//		// at start
//		FloatOutput allShooterMotors = flywheelMotorLeft.simpleControl().combine(
...
//		
		...
		// switching states
		shooterStates.setStateWhen("spinup", ControlBindings.spinFireButton.onPress().and(shooterStates.getIsState("inactive")));
		shooterStates.setStateWhen("firing", ControlBindings.spinFireButton.onPress().and(shooterStates.getIsState("spinup")));
...
//		
		...
		FloatInput flywheelTargetVelocity = shooterStates.selectByState(
				FloatInput.zero, // inactive
				flywheelShootingSpeed, // spinup
				flywheelShootingSpeed); // firing
...
////		Cluck.publish("Shooter Flywheel Velocity", flywheelVelocity);
		Cluck.publish("Shooter Flywheel Velocity", flywheelSpeed);
